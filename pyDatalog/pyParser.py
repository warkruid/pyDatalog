...
* Literal : made of a predicate and a list of arguments.  Instantiated when a symbol is called while executing the
  datalog program
    * HeadLiteral
    * Query
...
* Expression : base class for objects that can be part of an inequality, operation or slice
    ...
    * Operation : made of an operator and 2 operands. Instantiated when an operator is applied to a symbol while
    executing the datalog program
...
# call list of variables parsed since the last clause
...
class LazyListOfList(LazyList):
    ...
    def __unicode__(self):
        ...
        if self.data in (True, [], [()]):
            return util.unicode_type(self._data)
        # get the widths of each column
        widths = [max(len(util.unicode_type(x)) for x in column)
                      for column in zip(*(self._data))]
        widths = [max(widths[i], len(util.unicode_type(self.variables[i])))
                      for i in util.xrange(len(widths))]
        # get the formating string
        fofo = ' | '.join('%%-%ss' % widths[i]
                          for i in util.xrange(len(widths)))
    ...
    def __eq__(self, other):
        if isinstance(self, Operation) and self._pyD_operator in '+-' and self._pyD_lhs._pyD_value == 0:
            raise util.DatalogError(
                "Did you mean to assert or retract a fact ? Please add parenthesis.", None, None)
...
class Term(threading.local, Expression, LazyList):
    ...
    def __init__(self, name='??', forced_type=None):
        ...
        if isinstance(name, (list, tuple, util.xrange)):
            ...
            self._pyD_name=util.unicode_type(
                [element._pyD_name for element in self._pyD_value])
            self._pyD_type='tuple'
            self._pyD_lua=pyEngine.Term_of(
                [e._pyD_lua for e in self._pyD_value])
            ...
            index=name.find('.')
            if 0 < index:
                self._pyD_lua=pyEngine.Operation(pyEngine.Var(
                    name[:index]), '.',  pyEngine.Const(name[index:]))
            else:
    ...
    def __setitem__(self, keys, value):
        ...
        if Expression._pyD_for(keys)._pyD_lua.is_const() and value._pyD_lua.is_const():
            ...
            (function == function._pyD_symbol) <= (
                function._pyD_symbol == value)
...
class Function(Expression):
    ...
    def __init__(self, name, keys):
        ...
        self._pyD_precalculations=self._argument_precalculations & (
            self == self._pyD_symbol)
...
class Operation(Expression):
    ...
    def __init__(self, lhs, operator, rhs):
        ...
        self._pyD_lua=pyEngine.Operation(
            self._pyD_lhs._pyD_lua, self._pyD_operator, self._pyD_rhs._pyD_lua)
        self._pyD_precalculations=pre_calculations(
            (self._pyD_lhs, self._pyD_rhs))  # TODO test for slice, len
...
class Literal(object):
    ...
    def __init__(self, predicate_name, args, kwargs, prearity=None, aggregate=None):
        ...
        cls_name=self.predicate_name.split('.')[0].replace(
            '~', '') if 1 < len(self.predicate_name.split('.')) else ''
        if pyEngine.Class_dict.get(cls_name) and 2 <= len(self.args) and not isinstance(self.args[1], Term) \
            ...
            raise TypeError(
                "Object is incompatible with the class that is queried.")  # prefixed

        self.terms=[]  # the list of args converted to Expression
        for arg in self.args:
            if isinstance(arg, Literal):
                raise util.DatalogError("Syntax error: Literals cannot have a literal as argument : %s%s" % (
                    self.predicate_name, self.terms), None, None)
            elif isinstance(arg, Aggregate):
                raise util.DatalogError(
                    "Syntax error: Incorrect use of aggregation.", None, None)
        ...
        for term in self.terms:
            for var in term._pyD_variables().keys():
                # call update the list of variables since the last clause
                Thread_storage.variables.add(var)

        tbl=[a._pyD_lua for a in self.terms]
        # now create the literal for the head of a clause
        self.lua=pyEngine.Literal(
            self.predicate_name, tbl, self.prearity, aggregate)
    ...
    def make_for_comparison(cls, self, operator, other):
        ...
        if isinstance(self, Function):
            ...
            terms=[Term.make_for_prefix(
                self._pyD_name)] + list(self._pyD_keys) + [other]  # prefixed
            literal=Query(name, terms, {}, prearity)
            return self._argument_precalculations & other._pyD_precalculations & literal
        else:
            if not isinstance(other, Expression):
                raise util.DatalogError(
                    "Syntax error: Term or Expression expected", None, None)
    ...
    def literals(self):
        ...
        # pred ignore variables of negated literals
        ...
        if body is None:
            ...
            for literal in body.literals:
                if isinstance(literal, HeadLiteral):
                    raise util.DatalogError(
                        "Aggregation cannot appear in the body of a clause", None, None)
...
class Query(Literal, LazyListOfList):
    ...
    def __init__(self, predicate_name, terms, kwargs=None, prearity=None, aggregate=None):
        ...
        Literal.__init__(self, predicate_name, terms,
                         kwargs, prearity, aggregate)
    ...
    def __pos__(self):
        ...
        if self._variables():
            raise util.DatalogError(
                "Cannot assert a fact containing Variables", None, None)
    ...
    def __neg__(self):
        ...
        if self._variables():
            raise util.DatalogError(
                "Cannot retract a fact containing Variables", None, None)
        clause=pyEngine.Clause(self.lua, [])
...
class Body(LazyListOfList):
    ...
    def __init__(self, *args):
        ...
        self.literals=[]
        for arg in args:
            self.literals += [arg] if isinstance(arg,
                                                 Literal) else arg.literals
    ...
    def literal(self):
        ...
        literal=Literal.make('_pyD_query' + util.unicode_type(Body.counter.next()),
                             list(self._variables().values()), {}, prearity=prearity)
        literal <= self
        return literal
    ...
    def ask(self):
        ...
        transposed=list(zip(*(self._data))) if isinstance(self._data,
                        list) else None  # transpose result
...
class _transform_ast(ast.NodeTransformer):
    ...
    def visit_Compare(self, node):
        ...
        if 1 < len(node.comparators):
            raise util.DatalogError(
                "Syntax error: please verify parenthesis around (in)equalities", node.lineno, None)
        ...
        if sys.version_info < (3, 5):
            newNode=ast.Call(
                    ast.Attribute(var, 'in_' if isinstance(
                        node.ops[0], ast.In) else 'not_in_', var.ctx),  # func
                    [comparators],  # args
                    [],  # keywords
                    None,  # starargs
                    None  # kwargs
...
def add_program(func):
    ...
    defined=set(code.co_varnames).union(set(newglobals.keys()))
