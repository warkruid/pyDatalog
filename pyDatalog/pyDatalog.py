    ...
    def __call__(self, *arguments, **keyargs):
        for a in arguments:
            if isinstance(a, pyParser.Expression):
                assert not keyargs, "Sorry, key word arguments are \
...
ATOMS = ['_sum', 'sum_', '_min', 'min_', '_max', 'max_', '_len',
         'len_', 'concat', 'concat_', 'rank', 'rank_',
         'running_sum', 'running_sum_', 'range_', 'tuple_', 'format_',
         'mean_', 'linear_regression_']
        ...
        for arg in set(args + ATOMS):
            ...
            if words[0] in b:  # if it 's a builtin
                ...
                if len(words) == 2:  # e.g. kkecxivarenx.len
                    raise util.DatalogError("Unknown variable : %s" % words[0],
                                            None, None)
...
class metaMixin(type):
    ...
    def __init__(cls, name, bases, dct):
        """
        when creating a subclass of Mixin, save the
        subclass in Class_dict.
        """
        super(metaMixin, cls).__init__(name, bases, dct)
        pyEngine.add_class(cls, name)
        cls.has_SQLAlchemy = any(base.__module__
